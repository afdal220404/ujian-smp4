...
@section('title', 'Arsip Soal Siswa ' . $mapel->nama_mapel)
...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div class="hidden md:block">
            <span class="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-sm font-bold border border-blue-100">
                <i class="bi bi-folder2-open"></i> Total: <span id="total-files-badge">{{ count($arsipSoalSiswas) }}</span> File
            </span>
        </div>
    </div>
    ...
    {{-- FORM UTAMA --}}
    <form action="{{ route('guru.mapel.arsip_soal_siswa.store', $mapel->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden relative">
            ...
            <div id="file-list-container" class="divide-y divide-gray-50">
                ...
                @forelse ($arsipSoalSiswas as $file)
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        let fileCounter = {{ count($arsipSoalSiswas) }};
        ...
        if (confirmBtn) {
            confirmBtn.addEventListener("click", function() {
                if (deleteId) {
                    ...
                    deleteForm.action = "{{ url('guru/arsip-soal-siswa') }}" + '/' + deleteId;
                    deleteForm.submit();
                }
            });
        }
